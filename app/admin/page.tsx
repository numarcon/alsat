...
const navigation: { id: AdminScreen; icon: string; label: string }[] = [
  ...
  { id: "system", icon: "◎", label: "Жүйе күйі" },
];
  ...
  return (
    <main className="admin-shell">
      <aside className="admin-sidebar">
        <Link className="admin-brand" href="/admin"><span>A</span><div><b>ALSAT</b><small>PLATFORM ADMIN</small></div></Link>
        <p className="admin-nav-label">БАСҚАРУ</p>
        <nav>{navigation.map((item) => <button key={item.id} className={screen === item.id ? "active" : ""} onClick={() => { setScreen(item.id); setSearch(""); }}><i>{item.icon}</i>{item.label}{item.id === "companies" && <em>{companies.length}</em>}</button>)}</nav>
        <div className="admin-sidebar-bottom"><span>NA</span><div><b>{adminName}</b><small>Платформа әкімшісі</small></div><button onClick={logout} title="Шығу">⇥</button></div>
      </aside>

      <section className="admin-main">
        <header className="admin-topbar"><div><small>ALSAT CONTROL CENTER</small><h1>{navigation.find((item) => item.id === screen)?.label}</h1></div><div className="admin-top-actions"><label>⌕<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Іздеу" /></label><button className="admin-refresh" onClick={loadPlatform} disabled={loading}>↻</button><span className="admin-live">● Жүйе жұмыс істеп тұр</span></div></header>
      ...
      <nav className="admin-mobile-nav">{navigation.slice(0, 4).map((item) => <button key={item.id} className={screen === item.id ? "active" : ""} onClick={() => setScreen(item.id)}><i>{item.icon}</i><small>{item.label}</small></button>)}</nav>
    </main>
  );
}
...
function AdminLogin({ onSuccess }: { onSuccess: () => Promise<void> }) {
  ...
  return <main className="admin-auth"><section><Link className="admin-brand" href="/promo"><span>A</span><div><b>ALSAT</b><small>PLATFORM ADMIN</small></div></Link><p className="admin-auth-kicker">ҚОРҒАЛҒАН БӨЛІМ</p><h1>Платформаны басқару</h1><p>Компаниялар, пайдаланушылар, тапсырыстар және жүйе күйін бір жерден бақылаңыз.</p><form onSubmit={submit}><label>Email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="admin@alsat.kz" required /></label><label>Құпия сөз<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="••••••••" required /></label><button disabled={loading}>{loading ? "Тексерілуде…" : "Admin панельге кіру"}</button></form>{error && <div className="admin-login-error">{error}</div>}<Link className="admin-back" href="/workspace-login">← Workspace кіру бетіне қайту</Link></section><aside><div className="admin-auth-visual"><span>● LIVE</span><b>Барлық процесс<br/>бір бақылау орталығында.</b><div><i>◇</i><p><strong>Компаниялар</strong><small>Тіркелу және белсенділік</small></p></div><div><i>▤</i><p><strong>Тапсырыстар</strong><small>Сатудан жеткізуге дейін</small></p></div><div><i>◎</i><p><strong>Жүйе күйі</strong><small>Supabase және PWA</small></p></div></div></aside></main>;
}

function AdminLoading() { return <main className="admin-loading"><span>A</span><b>ALSAT ADMIN</b><p>Деректер тексерілуде…</p></main>; }
function AdminGate({ title, text, action, onAction }: { title: string; text: string; action: string; onAction: () => void | Promise<void> }) { return <main className="admin-gate"><span>A</span><small>ALSAT PLATFORM ADMIN</small><h1>{title}</h1><p>{text}</p><button onClick={onAction}>{action}</button><Link href="/promo">Промо парақшаға қайту</Link></main>; }
