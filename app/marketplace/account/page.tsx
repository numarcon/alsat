...
export default function MarketplaceAccountPage() {
  ...
  if (!user) return <main className="buyer-auth-shell">
    <section className="buyer-auth-story"><Link href="/marketplace" className="buyer-logo"><AlsatBrand inverse/></Link><div><span className="auth-kicker">БИЗНЕСКЕ АРНАЛҒАН САТЫП АЛУ ОРТАСЫ</span><h1>Сатып алуды<br/>бір кабинеттен<br/><em>басқарыңыз.</em></h1><p>Бірнеше жеткізушіден тапсырыс беріңіз, құжаттарды сақтаңыз және жеткізуді қоймадағы QR кезеңінен соңғы нүктеге дейін бақылаңыз.</p><ul><li><AlsatIcon name="shield" size={17}/>Көтерме бағалар мен тексерілген жеткізушілер</li><li><AlsatIcon name="cart" size={17}/>Бір себет — әр жеткізушіге қауіпсіз жеке тапсырыс</li><li><AlsatIcon name="route" size={17}/>Тапсырыс, төлем және жеткізу тарихы</li></ul></div></section>
  ...
  return <main className="buyer-account-shell">
    <aside className="buyer-sidebar"><Link href="/marketplace" className="buyer-logo"><AlsatBrand inverse/></Link><nav><button className={tab === "overview" ? "active" : ""} onClick={() => setTab("overview")}><i><AlsatIcon name="home"/></i>Шолу</button><button className={tab === "orders" ? "active" : ""} onClick={() => setTab("orders")}><i><AlsatIcon name="orders"/></i>Тапсырыстар <b>{activeOrders.length}</b></button><button className={tab === "favorites" ? "active" : ""} onClick={() => setTab("favorites")}><i><AlsatIcon name="heart"/></i>Таңдаулылар</button><button className={tab === "profile" ? "active" : ""} onClick={() => setTab("profile")}><i><AlsatIcon name="building"/></i>Компания профилі</button></nav><div className="sidebar-business"><span>{profile.businessName.slice(0, 1) || "B"}</span><div><b>{profile.businessName || user.email}</b><small>B2B сатып алушы</small></div></div><Link className="workspace-route" href="/workspace-login"><AlsatIcon name="marketplace" size={17}/> Жеткізуші Workspace</Link><button className="buyer-signout" onClick={() => void signOut()}><AlsatIcon name="logout" size={17}/> Шығу</button></aside>
    <section className="buyer-content"><header><div><span>ALSAT MARKETPLACE</span><h1>{tab === "overview" ? "Қайырлы күн!" : tab === "orders" ? "Тапсырыстар" : tab === "favorites" ? "Таңдаулылар" : "Компания профилі"}</h1></div><div><Link href="/marketplace/catalog">Каталогқа өту <AlsatIcon name="arrow" size={16}/></Link><button aria-label="Хабарламалар"><AlsatIcon name="bell"/></button></div></header>{error && <div className="account-error global">{error}</div>}{message && <div className="account-message global">{message}<button onClick={() => setMessage("")}>×</button></div>}

      {tab === "overview" && <><section className="buyer-welcome"><div><span>B2B САТЫП АЛУ КАБИНЕТІ</span><h2>{profile.businessName || "Компания профилін толтырыңыз"}</h2><p>Тапсырыс, қойма QR, экспедитор және төлем статусы бір тізбекте.</p><button onClick={() => setTab("orders")}>Тапсырыстарды көру <AlsatIcon name="arrow" size={17}/></button></div><div className="welcome-metric"><small>Жалпы сатып алу</small><strong>{money.format(totalPurchased)}</strong><span>{orders.length} тапсырыс</span></div></section><section className="buyer-stat-grid"><article><span><AlsatIcon name="orders"/></span><div><small>Белсенді тапсырыстар</small><strong>{activeOrders.length}</strong></div></article><article><span><AlsatIcon name="truck"/></span><div><small>Жолдағы тапсырыстар</small><strong>{orders.filter((order) => ["shipped", "labeled", "ready"].includes(order.warehouseStatus)).length}</strong></div></article><article><span><AlsatIcon name="check"/></span><div><small>Жеткізілген</small><strong>{orders.filter((order) => order.status === "delivered").length}</strong></div></article><article><span><AlsatIcon name="heart"/></span><div><small>Таңдаулы тауарлар</small><strong>{favoriteIds.length}</strong></div></article></section><section className="account-panel"><div className="panel-head"><div><h2>Соңғы тапсырыстар</h2><span>Жеткізушілер мен логистика статусы</span></div><button onClick={() => setTab("orders")}>Барлығын көру</button></div><OrderList orders={orders.slice(0, 3)} expanded={expandedOrder} onExpand={setExpandedOrder} onCancel={(id) => void cancelOrder(id)} onReorder={reorder} busy={busy}/></section></>}
      ...
      {tab === "favorites" && <section className="account-panel"><div className="panel-head"><div><h2>Сақталған ұсыныстар</h2><span>Бағасын бақылап, кейін себетке қосыңыз</span></div><Link href="/marketplace/catalog">Каталогтан қосу</Link></div>{favorites.length ? <div className="account-favorite-grid">{favorites.map((product) => <article key={product.id}><Link href={`/marketplace/product/${encodeURIComponent(product.id)}`} className="favorite-visual">{product.imageUrl ? <img src={product.imageUrl} alt=""/> : <span><AlsatIcon name="package" size={32}/></span>}</Link><small>{product.brand || product.category}</small><Link href={`/marketplace/product/${encodeURIComponent(product.id)}`}><strong>{product.name}</strong></Link><b>{money.format(product.price)}</b><div><button onClick={() => addFavoriteToCart(product)}><AlsatIcon name="cart" size={16}/> Себетке қосу</button><button onClick={() => void removeFavorite(product)} aria-label="Өшіру"><AlsatIcon name="trash" size={16}/></button></div></article>)}</div> : <div className="account-empty"><span><AlsatIcon name="heart" size={30}/></span><strong>Таңдаулы тауар жоқ</strong><p>Қажетті тауарға жүрек белгісін басыңыз — ол осында сақталады.</p><Link href="/marketplace/catalog">Каталогты ашу</Link></div>}</section>}
    ...
    <nav className="buyer-mobile-nav"><button onClick={() => setTab("overview")} className={tab === "overview" ? "active" : ""}><AlsatIcon name="home"/><span>Шолу</span></button><button onClick={() => setTab("orders")} className={tab === "orders" ? "active" : ""}><AlsatIcon name="orders"/><span>Тапсырыс</span></button><Link href="/marketplace/catalog"><AlsatIcon name="catalog"/><span>Каталог</span></Link><button onClick={() => setTab("favorites")} className={tab === "favorites" ? "active" : ""}><AlsatIcon name="heart"/><span>Таңдаулар</span></button><button onClick={() => setTab("profile")} className={tab === "profile" ? "active" : ""}><AlsatIcon name="user"/><span>Профиль</span></button></nav>
  </main>;
}
