...
const popularCategories: ReadonlyArray<{ title: string; category: string; icon: AlsatIconName }> = [
  { title: "Өнеркәсіп жабдықтары", category: "Өнеркәсіп және бизнес", icon: "tools" },
  { title: "Құрылыс және жөндеу", category: "Құрылыс және жөндеу", icon: "package" },
  { title: "Электроника және техника", category: "Электроника және техника", icon: "laptop" },
  { title: "Кеңсе және қағаз өнімдері", category: "Үй, жиһаз және бақша", icon: "office" },
  { title: "Тұрмыстық тауарлар және химия", category: "Үй, жиһаз және бақша", icon: "cleaning" },
  { title: "Қаптама және ыдыстар", category: "Өнеркәсіп және бизнес", icon: "box" },
  { title: "Киім және қорғаныс құралдары", category: "Өнеркәсіп және бизнес", icon: "vest" },
  { title: "Азық-түлік және сусындар", category: "Азық-түлік және сусындар", icon: "food" },
];

const trustBenefits: ReadonlyArray<{ icon: AlsatIconName; title: string; text: string }> = [
  { icon: "shield", title: "Сенімді жеткізушілер", text: "Тексерілген серіктестер" },
  { icon: "commission", title: "Ыңғайлы шарттар", text: "Көтерме және келісімді баға" },
  { icon: "truck", title: "Жылдам жеткізу", text: "QR арқылы бақыланады" },
  { icon: "lock", title: "Қауіпсіз төлем", text: "Құжатталған B2B сатып алу" },
];

const platformBenefits: ReadonlyArray<{ icon: AlsatIconName; title: string; text: string }> = [
  { icon: "building", title: "B2B үшін арнайы", text: "Шағын және орта бизнеске арналған шешімдер" },
  { icon: "catalog", title: "Кең таңдау", text: "Мыңдаған тауар және жеткізушілер" },
  { icon: "user", title: "Жеке менеджер", text: "Сізге жеке қолдау көрсетеміз" },
  { icon: "route", title: "Біртұтас процесс", text: "Тапсырыстан жеткізуге дейін бір жүйе" },
];
  ...
  return <main className="marketplace-shell">
    <div className="marketplace-page">
      <header className="marketplace-header">
        <Link className="marketplace-logo" href="/marketplace" aria-label="Alsat Marketplace басты беті"><AlsatBrand /></Link>
        <Link className="catalog-menu-button" href="/marketplace/catalog"><AlsatIcon name="menu" size={18}/>Каталог</Link>
        <label className="marketplace-search"><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if(event.key === "Enter") window.location.href=`/marketplace/catalog?q=${encodeURIComponent(query)}`; }} placeholder="Тауар, бренд немесе категорияны іздеңіз"/><AlsatIcon name="search" size={19}/></label>
        <div className="marketplace-header-actions"><button className="language-button" aria-label="Тілді таңдау">KZ<span>⌄</span></button><Link className="header-icon" href="/marketplace/account?tab=favorites" aria-label="Таңдаулылар"><AlsatIcon name="heart"/></Link><button className="header-icon header-cart" onClick={() => setCartOpen(true)} aria-label="Себет"><AlsatIcon name="cart"/>{cartCount > 0 && <b>{cartCount}</b>}</button><Link className="marketplace-login" href="/marketplace/account">Кабинетке кіру <AlsatIcon name="arrow" size={17}/></Link></div>
      </header>

      <div className="mobile-search-row"><label className="marketplace-search"><input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if(event.key === "Enter") window.location.href=`/marketplace/catalog?q=${encodeURIComponent(query)}`; }} placeholder="Тауарларды іздеу"/><AlsatIcon name="search" size={19}/></label></div>

      <section className="marketplace-hero">
        <div className="hero-copy"><span className="hero-eyebrow"><i/>Қазақстан бизнесіне арналған B2B экожүйе</span><h1>Бизнесіңе қажетінің<br/><em>бәрі бір жерде.</em></h1><p>Сенімді жеткізуші, көтерме баға және құжатталған логистика — сатып алудан жеткізуге дейін бір жүйеде.</p><div className="hero-actions desktop-hero-actions"><Link className="primary-cta" href="/marketplace/catalog">Каталогты ашу <AlsatIcon name="arrow" size={18}/></Link><Link className="secondary-cta" href="/workspace-signup">Жеткізуші болу</Link></div><div className="hero-metrics"><span><strong>1 жүйе</strong><small>сауда мен логистика</small></span><span><strong>QR</strong><small>бақыланатын жеткізу</small></span><span><strong>B2B</strong><small>бизнес құжаттары</small></span></div></div>
        <div className="hero-visual"><div className="hero-orbit hero-orbit-one"/><div className="hero-orbit hero-orbit-two"/><img src="/marketplace/hero-container-v1.png" alt="Жасыл жүк контейнері және тауар қораптары"/><div className="container-brand"><AlsatMark size={35}/><b>ALSAT<small>MARKETPLACE</small></b></div><div className="hero-status-card"><span><AlsatIcon name="shield" size={18}/></span><div><b>Тексерілген сауда</b><small>Workspace арқылы расталған</small></div></div></div>
        <div className="hero-actions mobile-hero-actions"><Link className="primary-cta" href="/marketplace/catalog">Каталогты ашу <AlsatIcon name="arrow" size={18}/></Link><Link className="secondary-cta" href="/workspace-signup">Жеткізуші болу</Link></div>
      </section>

      <section className="trust-strip" id="benefits">{trustBenefits.map((item) => <article key={item.title}><i><AlsatIcon name={item.icon}/></i><div><strong>{item.title}</strong><span>{item.text}</span></div><b><AlsatIcon name="chevron" size={16}/></b></article>)}</section>
      ...
      <section className="marketplace-section offers-section" id="offers">
        ...
        <div className="showcase-grid">{visibleProducts.slice(0, 5).map((product, index) => <article className="showcase-card" key={product.id}><button className={favorites.includes(product.id) ? "favorite-button active" : "favorite-button"} onClick={() => void toggleFavorite(product)} aria-label="Таңдаулыларға қосу"><AlsatIcon name="heart"/></button><Link className={product.imageUrl ? "showcase-visual has-image" : `showcase-visual showcase-art-${index % 5}`} href={`/marketplace/product/${encodeURIComponent(product.id)}`} aria-label={`${product.name} парақшасын ашу`}>{product.imageUrl && <img src={product.imageUrl} alt=""/>}<span className="product-card-view">Толық көру <AlsatIcon name="arrow" size={15}/></span></Link><div className="showcase-copy"><small>{product.brand || product.category}</small><h3><Link className="showcase-title-link" href={`/marketplace/product/${encodeURIComponent(product.id)}`}>{product.name}</Link></h3><span>{product.minOrder > 1 ? `Мин. ${product.minOrder} ${product.unit}` : product.subcategory}</span><strong>{money.format(product.price)}</strong><button onClick={() => addToCart(product)}><AlsatIcon name="cart" size={17}/> Себетке қосу</button></div></article>)}</div>
      ...
      <footer className="marketplace-footer">
        <div className="footer-grid"><div className="footer-brand"><Link className="marketplace-logo light" href="/marketplace"><AlsatBrand inverse/></Link><p>Сатып алу, сату және логистикаға арналған Қазақстандық B2B экожүйе.</p><div className="footer-trust"><AlsatIcon name="shield" size={17}/><span>Workspace арқылы тексерілген сауда</span></div></div><div><strong>Компания</strong><Link href="/promo">Біз туралы</Link><Link href="/workspace-signup">Жеткізушілерге</Link><Link href="/legal/terms">Платформа ережелері</Link><Link href="/legal/offer">Жария оферта</Link></div><div><strong>Көмек</strong><Link href="/legal/payment">Төлем және қауіпсіздік</Link><Link href="/legal/delivery">Жеткізу шарттары</Link><Link href="/legal/refund">Қайтару және бас тарту</Link><Link href="/legal/contacts">Байланыс</Link></div><div><strong>Жеке кабинет</strong><Link href="/marketplace/account?tab=orders">Тапсырыстарым</Link><Link href="/marketplace/account?tab=favorites">Таңдаулылар</Link><Link href="/marketplace/account">Компания профилі</Link><Link href="/workspace-login">Workspace</Link></div><div className="footer-contacts"><strong>Байланыс</strong><a href="tel:+77003003009"><AlsatIcon name="phone" size={15}/>+7 (700) 300-30-09</a><a href="mailto:info@alsat.kz"><AlsatIcon name="mail" size={15}/>info@alsat.kz</a><span><AlsatIcon name="location" size={15}/>Алматы қ., Айналмалы көшесі, 69А</span><span><AlsatIcon name="clock" size={15}/>Дс–Жм: 09:00–18:00</span></div></div>
    ...
    <nav className="mobile-marketplace-nav"><a href="#" className="active"><i><AlsatIcon name="home"/></i><span>Басты бет</span></a><Link href="/marketplace/catalog"><i><AlsatIcon name="catalog"/></i><span>Каталог</span></Link><button onClick={() => setCartOpen(true)}><i><AlsatIcon name="cart"/></i><span>Себет</span>{cartCount > 0 && <b>{cartCount}</b>}</button><Link href="/marketplace/account?tab=favorites"><i><AlsatIcon name="heart"/></i><span>Таңдаулар</span></Link><Link href="/marketplace/account"><i><AlsatIcon name="user"/></i><span>Профиль</span></Link></nav>
    {notice && <div className="marketplace-toast">{notice}<button onClick={() => setNotice("")}>×</button></div>}

    {cartOpen && <div className="marketplace-overlay" onClick={() => setCartOpen(false)}><aside className="marketplace-cart" onClick={(event) => event.stopPropagation()}><div className="cart-head"><div><span className="marketplace-kicker">ALSAT MARKETPLACE</span><h2>Себет</h2></div><button className="modal-close" onClick={() => setCartOpen(false)}><AlsatIcon name="close" size={18}/></button></div>{cartLines.length ? <><div className="cart-lines">{cartLines.map(({ product, quantity }) => <div className="cart-line" key={product.id}><div className="cart-line-visual"><AlsatIcon name="package"/></div><div className="cart-line-copy"><strong>{product.name}</strong><small>{money.format(product.price)}</small><div className="quantity-control"><button onClick={() => changeQuantity(product, -1)}><AlsatIcon name="minus" size={15}/></button><b>{quantity}</b><button onClick={() => changeQuantity(product, 1)}><AlsatIcon name="plus" size={15}/></button></div></div><strong>{money.format(product.price * quantity)}</strong></div>)}</div><div className="cart-summary"><span>Барлығы <b>{cartCount} дана</b></span><strong>{money.format(cartTotal)}</strong></div><button className="modal-primary" onClick={() => void openCheckout()} disabled={checkoutLoading}>{checkoutLoading ? "Тексерілуде…" : <>Тапсырысты рәсімдеу <AlsatIcon name="arrow" size={18}/></>}</button><small className="cart-hint">Тапсырыс жасалғанда ол бірден Alsat қоймасының жұмыс тізбегіне түседі.</small></> : <div className="cart-empty"><span><AlsatIcon name="cart" size={34}/></span><strong>Себет бос</strong><small>Каталогтан тауар қосыңыз.</small><button className="modal-primary" onClick={() => setCartOpen(false)}>Каталогқа оралу</button></div>}</aside></div>}
