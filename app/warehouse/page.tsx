...
export default function WarehouseApp() {
  ...
  return <main className="qmart-role warehouse-shell">
    <header className="role-header"><button onClick={() => go("more")}><AlsatIcon name="menu"/></button><div className="role-header-brand"><AlsatMark size={27}/><span><b>ALSAT</b><small>ҚОЙМА МЕНЕДЖЕРІ</small></span></div><button onClick={() => go("notifications")}><AlsatIcon name="bell"/></button></header>
    ...
    <nav className="role-bottom"><button className={screen === "dashboard" ? "active" : ""} onClick={() => go("dashboard")}><AlsatIcon name="home"/><small>Басты</small></button><button className={screen === "products" || screen === "product" || screen === "stock" ? "active" : ""} onClick={() => go("products")}><AlsatIcon name="package"/><small>Тауарлар</small></button><button className={screen === "receive" || screen === "issue" || screen === "transfer" || screen === "return" ? "active" : ""} onClick={() => go("receive")}><AlsatIcon name="scan"/><small>Операциялар</small></button><button className={screen === "orders" ? "active" : ""} onClick={() => go("orders")}><AlsatIcon name="orders"/><small>Тапсырыстар</small></button><button className={screen === "more" || screen === "profile" ? "active" : ""} onClick={() => go("more")}><AlsatIcon name="settings"/><small>Көбірек</small></button></nav>
  </main>;
}
...
function WarehouseLoginAlsat({ onLogin }: { onLogin: () => void }) { return <main className="role-login warehouse-login"><div className="role-login-brand"><span><AlsatMark size={31}/></span><b>ALSAT</b><small>ҚОЙМА МЕНЕДЖЕРІ</small></div><h1>Жүйеге кіру</h1><p>Аккаунтыңызға кіріп, қойма жұмысыңызды басқарыңыз</p><label>Телефон нөмірі<input placeholder="+7 (___) ___-__-__" inputMode="tel"/></label><label>Құпия сөз<input placeholder="••••••••••••" type="password"/></label><div className="login-check"><span>☑　Мені есте сақтау</span><a>Құпия сөзді ұмыттыңыз ба?</a></div><button className="role-primary" onClick={onLogin}>Кіру</button><div className="role-or">немесе</div><button className="role-secondary" onClick={onLogin}><AlsatIcon name="phone" size={17}/> SMS арқылы кіру</button></main> }
