...
function WarehouseReports({ go }: { go: (screen: Screen) => void }) { const [selected,setSelected]=useState("");return <section className="role-screen"><div className="role-heading"><h1>Есеп</h1><button onClick={()=>window.print()}>⎙</button></div>{["Қалдық есебі","Қозғалыс есебі","Тауарлардың қозғалысы","Қабылдау есебі","Шығару есебі","Қалдық бойынша ABC талдау","Қоймадағы тұрған тауарлар"].map((name)=><button className="setting-row role-setting" key={name} onClick={()=>setSelected(name)}>▣　{name}<b>›</b></button>)}{selected&&<div className="action-panel"><strong>{selected}</strong><p>Есеп дайын. Экраннан қарауға немесе басып шығаруға болады.</p><div className="store-form-actions"><button onClick={()=>go("stock")}>Деректерді ашу</button><button onClick={()=>window.print()}>Басып шығару</button></div></div>}</section> }
