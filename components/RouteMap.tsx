...
export default function RouteMap({ stops }: { stops: MapStop[] }) {
  ...
  useEffect(() => {
    ...
    if (stops.length > 1) fetch(`https://api.mapbox.com/directions/v5/mapbox/driving/${coordinates}?geometries=geojson&overview=full&access_token=${token}`).then(response => response.json()).then(data => { const geometry = data.routes?.[0]?.geometry; if (!geometry) return; map.on("load", () => { if (map.getSource("daily-route")) return; map.addSource("daily-route", { type: "geojson", data: { type: "Feature", properties: {}, geometry } }); map.addLayer({ id: "daily-route-line", type: "line", source: "daily-route", paint: { "line-color": "#159143", "line-width": 4, "line-opacity": 0.85 } }); }); }).catch(() => undefined);
    return () => map.remove();
  }, [stops, token]);
